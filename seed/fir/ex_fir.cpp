...
static void InitWindow()
{
    ...
    for (size_t i = 0; i < flt_size/2; i++)
    {
        ...
        wnd[i] = a0 - a1 * cosf(wind_arg) + a2 * cosf(wind_arg * 2) - a3 * cosf(wind_arg * 3);
    }
}
...
int main(void)
{
    ...
    while(1) 
    {
        ...
        {
            const float rate = (float)update_count * callback_rate / callback_count;
            seed.PrintLine("Average rate = " FLT_FMT3 "upd/sec", FLT_VAR3(rate));
        }
