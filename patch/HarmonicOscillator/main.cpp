...
DaisyPatch patch;               // Make a patch
MorphOscillator oscillators[4]; // make four oscillators
float oscFreqs[4] = {0};
int mode = 0;
int ratioIndex = 0;
const char* currentRatioLabel = "";
...
void DisplayCurrentMode() {
    ...
    std::string modeName = "Symmetric Limit 5";
    ...
    patch.display.WriteString(currentRatioLabel, Font_7x10, true);
...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size) {
    ...
    ratioIndex = powf(patch.GetKnobValue(DaisyPatch::CTRL_2), 2.0f) * (kLimit5SymmetricalSize - 1);
    ratioIndex = fminf(fmaxf(ratioIndex, 0), kLimit5SymmetricalSize - 1);
    currentRatioLabel = kLimit5Symmetrical[ratioIndex].label;

    for (int i = 0; i < 4; i++) {
        int rIdx = (ratioIndex + i) % kLimit5SymmetricalSize;
        float freq = baseFreq * kLimit5Symmetrical[rIdx].value;
