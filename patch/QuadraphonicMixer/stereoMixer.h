...
namespace QuadraphonicMixer
{
    ...
    struct StereoMixer
    {
        ...
        void Process(const Input& in, Output& out) {
            ...
            {
                out.Values[n] = 
                    ProcessChannel(in.Values[0], 0, n) +
        ...
        inline float GetAngle(const uint8_t index) const
        {
            return _angle[index];
        }

        inline float GetAmp(const uint8_t channelIndex, const uint8_t outIndex) const
        {
            return _amp[channelIndex][outIndex];
        }

    private:
        inline float ProcessChannel(const float value, const uint8_t channelIndex, const uint8_t outIndex) {
            return _amp[channelIndex][outIndex] * value;
        }

        float _angle[STEREO_MIXER_NUM_CHANNEL]{0.f, 0.f, 0.f, 0.f};
        float _amp[STEREO_MIXER_NUM_CHANNEL][STEREO_MIXER_NUM_OUTPUT]{ 0.f, 0.f, 0.f, 0.f };
    };
}
#endif